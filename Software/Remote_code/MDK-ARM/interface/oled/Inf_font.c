 ...
 * @date    2025-09-27
 *
 * @details 本文件包含OLED显示模块使用的字库点阵数据：
 *          - oled_asc2_1206：6x12像素ASCII字库
 *          - oled_asc2_1608：8x16像素ASCII字库
 *          - oled_CH_1616：16x16像素中文字符字库
 *
 * 字库说明：
 * - ASCII字库：95个字符（从空格' '~'~'），偏移量=字符-32
 * - 中文字库：半角存储，每个中文字符占12字节
 * - 字库数据由取模软件生成，阴码取模，列行式，逆向
 *
 * @note   字库数据存储在Flash中（const修饰）
 * @note   修改字库需使用取模软件重新生成
 */
 ...
 * @brief 6x12像素ASCII字库
 *
 * @details 95个可显示ASCII字符（0x20~0x7E）
 *          - 每字符12字节
 *          - 字符宽度6像素，高度12像素
 *          - 取模方式：阴码，列行式，逆向
 *
 * @note   偏移量=ASCII码-32（空格为0）
 * @note   共95个字符，覆盖空格到'~'
 */
...
const unsigned char oled_CH_1616[][12] = {
    ...
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},/*空白,0*/

    {0x84, 0x10, 0x47, 0xE0, 0x00, 0x10, 0x03, 0xD0, 0xFA, 0x10, 0xAA, 0x90},
    {0xFF, 0x90, 0xAA, 0x90, 0xAB, 0x90, 0xFA, 0x50, 0x03, 0xD0, 0x00, 0x00},

    /* 中文字符：遥控 */
    {0x00, 0x10, 0x7F, 0x10, 0x40, 0x20, 0x40, 0x40, 0x41, 0x80, 0x5E, 0x00},
    {0x41, 0xE0, 0x40, 0x10, 0x40, 0x10, 0x7F, 0x10, 0x00, 0x70, 0x00, 0x00},

    /* 中文字符：司 */
    {0x10, 0x00, 0x57, 0xE0, 0x54, 0x40, 0x54, 0x40, 0x54, 0x40, 0x54, 0x40},
    {0x57, 0xD0, 0x50, 0x10, 0x40, 0x10, 0x7F, 0xE0, 0x00, 0x00, 0x00, 0x00},

    /* 中文字符：航 */
    {0x30, 0x10, 0x25, 0x10, 0x29, 0x10, 0x31, 0x10, 0xA1, 0x10, 0x61, 0xF0},
    {0x21, 0x10, 0x31, 0x10, 0x29, 0x10, 0x25, 0x10, 0x30, 0x10, 0x00, 0x00},

    /* 空白字符 */
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},
    {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00},/*空白,5*/

    /* ======================== 信号图标 ======================== */

    /* 信号图标：无信号 */
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x80, 0x09, 0x00, 0x06, 0x00},
    {0x06, 0x00, 0x09, 0x00, 0x10, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, /*无信号,0*/

    /* 信号图标：一格信号 */
    {0x00, 0x00, 0x00, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, /*一格,1*/

    /* 信号图标：两格信号 */
    {0x00, 0x00, 0x00, 0x30, 0x00, 0x00, 0x00, 0xF0, 0x00, 0x00, 0x00, 0x00},
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, /*两格,2*/

    /* 信号图标：三格信号 */
    {0x00, 0x00, 0x00, 0x30, 0x00, 0x00, 0x00, 0xF0, 0x00, 0x00, 0x03, 0xF0},
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, /*三格,3*/

    /* 信号图标：四格信号 */
    {0x00, 0x00, 0x00, 0x30, 0x00, 0x00, 0x00, 0xF0, 0x00, 0x00, 0x03, 0xF0},
    {0x00, 0x00, 0x0F, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, /*四格,4*/

    /* 信号图标：五格信号（满格） */
    {0x00, 0x00, 0x00, 0x30, 0x00, 0x00, 0x00, 0xF0, 0x00, 0x00, 0x03, 0xF0},
    {0x00, 0x00, 0x0F, 0xF0, 0x00, 0x00, 0x3F, 0xF0, 0x00, 0x00, 0x00, 0x00}, /*五格,5*/
    ...
    {0x28, 0x20, 0x09, 0xE0, 0x2A, 0x00, 0x4C, 0x00, 0x00, 0x00, 0x00, 0x00}, /*WiFi,19*/

    /* 电池图标 */
    {0x00, 0x00, 0x40, 0x20, 0x26, 0x40, 0x15, 0x80, 0x0D, 0xE0, 0x16, 0x20},
    {0x16, 0x20, 0x0D, 0xE0, 0x15, 0x80, 0x26, 0x40, 0x40, 0x20, 0x00, 0x00}, /*电池,20*/
